(async function () {
  ...
  function getCorrectCpms(floatCPMs, resultType = "df") {
    if (floatCPMs.length == 0) {
      console.error("floatCpms array is empty");
      return;
    }
  ...
  const getTrueCpmsFromCalcCpms_gpuC = gpuC.createKernel(
    function (calcCpms, cpms, cpmCount) {
      const { x, y } = this.thread;
      /* 
        x -> iterator for pokeform
        y -> iterator for IVs
       */
      for (let i = cpmCount; i > -1; i--) {
        if (cpms[i] <= calcCpms[y][x]) return cpms[i];
      }
      return cpms[0];
    },
    {
  ...
  const getPokeForms = async (filePath) =>
  ...
  try {
    pokeForms = (await getPokeForms(pokeForms_liveServer)).data;
  } catch (err) {
    // console.error(err)
    throw err;
  }

  const pfHeaders = pokeForms[0];
  ...
  pokeForms = GPU.input(pokeForms.flat(), [pokeForms[0].length, pokeForms.length])
  ivs = GPU.input(ivs.flat(), [ivs[0].length, ivs.length])
  cpmsFloatArray = GPU.input(cpmsFloatArray.flat(), [cpmsFloatArray[0].length, cpmsFloatArray.length])
  // console.log(cpmsFloatArray);
