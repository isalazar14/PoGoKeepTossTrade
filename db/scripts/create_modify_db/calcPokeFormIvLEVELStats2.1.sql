...
WITH
  ...
  , GL_candidatesStatProds as (
		select pokemon_form_id, iv_id, level_id as gl_level_id, cp as gl_cp, hp as gl_hp
			-- , round((round(atk *multiplier,2) * round(def *multiplier,2) * floor(sta *multiplier)) /1000,2) as gl_stat_product -- original stat_prod formula, not not 100% accurate, probs due to rounding inside
      , round((atk *multiplier) * (def *multiplier) * floor(sta *multiplier) /1000, 3) as gl_stat_product
    from IvLevelCPs
  ...
  , GL_IvsRankedAdvanced as (
	select pokemon_form_id, iv_id, gl_level_id, gl_cp, gl_hp, gl_stat_product, gl_rank
		, round(gl_stat_product / gl_max_sp * 100, 3) as `gl_%_max_sp`
        , (4096-gl_rank)/4096 * 100 AS gl_percent_rank
  ...
  , UL_candidatesStatProds as (
		select pokemon_form_id, iv_id, level_id as ul_level_id, cp as ul_cp, hp as ul_hp
			-- , round((round(atk *multiplier,2) * round(def *multiplier,2) * floor(sta *multiplier)) /1000,2) as ul_stat_product -- original stat_prod formula, not not 100% accurate, probs due to rounding inside
			, round((atk *multiplier) * (def *multiplier) * floor(sta *multiplier) /1000, 3) as ul_stat_product
    from IvLevelCPs
  ...
  , UL_IvsRankedAdvanced as (
	select pokemon_form_id, iv_id, ul_level_id, ul_cp, ul_hp, ul_stat_product, ul_rank
		, round(ul_stat_product / ul_max_sp * 100, 3) as `ul_%_max_sp`
        , (4096-ul_rank)/4096 * 100 AS ul_percent_rank
