...
if (calcParams.IV_BATCH_SIZE < 4096)
    ivs.splice(calcParams.IV_BATCH_SIZE);
// console.log(ivs)
// prettier-ignore
...
let pokeForms;
try {
    let pokeForm_csv = await getPokeForms(calcParams.POKEFORMS_CSV_LOCATION_URL, pfOptions);
...
const pokeCalc = {
    ...
    cpmsAtCpLimit: {
        onCPU: function (cpLimit, pokeForms, ivs) {
...
const calcTimeCell_calcStats = getCalcTimeRowCell(calcTimeRow_calcStats, "operation");
...
function runCalcPerfTest(calcName, calc, runOptions) {
    if (!runOptions?.silentRun)
    ...
    if (runOptions != undefined) {
        if (runOptions.renderPerformance) {
            ...
            if (runOptions.renderPerformance.appendToEl)
                runOptions.renderPerformance.appendToEl.append(runOptions.renderPerformance.targetEL);
            else if (runOptions.renderPerformance.appendChildToEl)
                runOptions.renderPerformance.appendChildToEl.appendChild(runOptions.renderPerformance.targetEL);
        }
    }
    ...
    if (runOptions != undefined) {
        if (runOptions.logPerformance == "full")
            ...
            console.log(`${calcName} duration: ${Math.round(perfResult.duration).toLocaleString()} ms`);
        if (runOptions.logResult)
            console.log(`${calcName} performance:\n`, perfResult);
        if (runOptions.renderPerformance) {
            runOptions.renderPerformance.targetEL.innerHTML = Math.round(perfResult.duration).toLocaleString();
            runOptions.renderPerformance.targetEL.classList.toggle("running-calc");
        }
    }
    if (!runOptions?.silentRun)
        console.info("background-color:orange; color:black");
