...
console.log("in gpuTest.ts/js");
const POKEFORMS_CSV_LOCATION_URL = {
    expressServer: "http://localhost:8000/data/pokemon_forms.csv",
    liveServer: "http://localhost:5500/client/src/data/csv/pokemon_forms.csv",
};
let pfHeaders;
let pokeForms;
try {
    let pokeForm_csv = await getPokeForms(POKEFORMS_CSV_LOCATION_URL.liveServer, {
        // preview: 5,
        pIdRange: [25, 26],
        // pfSelection: ["25,1", "26,1"],
    });
...
let options = {
    PF_BATCH_SIZE: pokeForms.length,
    IV_BATCH_SIZE: 4096,
    MAX_LEVEL: 40,
    CP_LIMIT: 1500
};
...
function calcCpmsAtCpLimit_cpu(cpLimit, pokeForms, ivs) {
    ...
    let i = 0 /* iterator for calcCPMs */, j = 0; /* iterator for  */
    for (let pf of pokeForms) {
        calcCpms[i] = Array(options.IV_BATCH_SIZE);
        for (let ivSet of ivs) {
            ...
            calcCpms[i][j] = Math.sqrt(((cpLimit + 0.9999999) * 10) / (a * Math.sqrt(d) * Math.sqrt(s)));
            j++;
        }
        i++;
        j = 0;
...
const calcCp_gpuG = gpuG
    .createKernel(function (pokeForms, ivs, cpms) {
    // prettyier-ignore
    /**
    ...
    .setOutput([cpms.length, options.IV_BATCH_SIZE, options.PF_BATCH_SIZE]); // [x, y, z] -> matches this.thread._
const calcCpmsAtCpLimit_gpuG = gpuG
    .createKernel(function (cpLimit, pokeForms, ivs) {
    const atkBaseIdx = 2, defBaseIdx = 3, staBaseIdx = 4;
    const atkIvIdx = 1, defIvIdx = 2, staIvIdx = 3;
    ...
    .setPipeline(true)
    .setOutput([options.IV_BATCH_SIZE, options.PF_BATCH_SIZE]);
...
const getTrueCpmsFromCalcCpms_cpu = (calcCpms, cpms) => {
    ...
    for (let y = 0; y < yLength; y++) {
        trueCpms[y] = Array(options.IV_BATCH_SIZE);
        for (let x = 0; x < options.IV_BATCH_SIZE; x++) {
            trueCpms[y][x] = cpms[0];
...
//  * @returns
//  */
function (calcCpms, cpms, lastCpmIdx, returnCPM) {
    // debugger;
    const { x, y } = this.thread;
    ...
    output: [options.IV_BATCH_SIZE, options.PF_BATCH_SIZE],
    // optimizeFLoatMemory: true,
    // tactic: "precision",
});
// .setOutput([options.IV_BATCH_SIZE, options.PF_BATCH_SIZE]);
const getTrueCpmsFromCalcCpms_gpuC = gpuC.createKernel(function (calcCpms, cpms, lastCpmIdx, returnCPM) {
    const { x, y } = this.thread;
    /*
        x -> iterator for pokeform
        y -> iterator for IVs
...
// .setOutput([options.IV_BATCH_SIZE, options.PF_BATCH_SIZE]);
// pokeForms = pokeForms.slice(0, options.PF_BATCH_SIZE);
...
// ivs = ivs.splice(500, options.IV_BATCH_SIZE);
// ivs = [[0, 1, 14, 12]];
let maxLevelIdx = options.MAX_LEVEL * 2 - 2;
...
// const calcCpms_cpu = calcCpmsAtCpLimit_cpu(options.CP_LIMIT, pokeForms, ivs);
...
const calcCpms_gpuG_texture = calcCpmsAtCpLimit_gpuG(options.CP_LIMIT, pokeForms, ivs);
...
// const calcCpms_gpuC = calcCpmsAtCpLimit_gpuC(options.CP_LIMIT, pokeForms, ivs);
        ...
        Papa.parse(filePath, {
            ...
            complete: (parseResults) => {
                ...
                if (!options || options?.preview) {
                    ...
                    result.data = parseResults.data.filter((pf) => pf[0] >= options.pIdRange[0] && pf[0] <= options.pIdRange[1]);
                }
                    ...
                    while (remaining > 0 && i < parseResultsLength) {
                        if (selection.has(`${parseResults.data[i][0]},${parseResults.data[i][1]}`)) {
